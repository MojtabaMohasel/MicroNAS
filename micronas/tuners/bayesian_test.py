...
from micronas.engine import hyperparameters as hp_module
from micronas.engine import trial as trial_module
from micronas.tuners import bayesian as bo_module
...
def test_scipy_not_install_error(tmp_path):
    scipy_module = micronas.tuners.bayesian.scipy
    micronas.tuners.bayesian.scipy = None

    with pytest.raises(ImportError, match="Please install scipy"):
        micronas.BayesianOptimization(
            hypermodel=build_model,
            directory=tmp_path,
        )

    micronas.tuners.bayesian.scipy = scipy_module


def test_sklearn_not_install_error(tmp_path):
    sklearn_module = micronas.tuners.bayesian.sklearn
    micronas.tuners.bayesian.sklearn = None
    ...
    micronas.tuners.bayesian.sklearn = sklearn_module
...
def test_bayesian_oracle(tmp_path):
    ...
    oracle = bo_module.BayesianOptimizationOracle(
        objective=micronas.Objective("score", "max"),
        max_trials=20,
...
def test_bayesian_oracle_maximize(tmp_path):
    ...
    oracle = bo_module.BayesianOptimizationOracle(
        objective=micronas.Objective("score", direction="max"),
        max_trials=20,
        hyperparameters=hps,
        num_initial_points=2,
    )
...
def test_float_optimization(tmp_path):
    class PolynomialTuner(micronas.engine.base_tuner.BaseTuner):
    ...
    tuner = PolynomialTuner(
        oracle=micronas.oracles.BayesianOptimizationOracle(
            objective=micronas.Objective("score", "max"),
            hyperparameters=hps,
            max_trials=50,
        ),
...
def test_distributed_optimization(tmp_path):
    ...
    oracle = bo_module.BayesianOptimizationOracle(
        objective=micronas.Objective("score", "min"),
        hyperparameters=hps,
        max_trials=60,
    )
    oracle._set_project_dir(tmp_path, "untitled")
    ...
    class MyTuner(bo_module.BayesianOptimization):
        def run_trial(self, trial, *args, **kwargs):
            ...
            if len(self.oracle.start_order) == 1:
                raise micronas.errors.FailedTrialError()
            return [np.random.rand()]
