...
def test_sklearn_not_install_error(tmp_path):
    sklearn_module = micronas.tuners.sklearn_tuner.sklearn
    micronas.tuners.sklearn_tuner.sklearn = None

    with pytest.raises(ImportError, match="Please install sklearn"):
        micronas.SklearnTuner(
            oracle=micronas.oracles.BayesianOptimizationOracle(
                objective=micronas.Objective("score", "max"), max_trials=10
            ),
            hypermodel=build_model,
    ...
    micronas.tuners.sklearn_tuner.sklearn = sklearn_module
