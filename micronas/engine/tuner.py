...
from micronas.engine import base_tuner
from micronas.engine import tuner_utils


@keras_tuner_export(
    [
        "micronas.Tuner",
        "micronas.tuners.Tuner",
        "micronas.engine.tuner.Tuner",
    ]
)
class Tuner(base_tuner.BaseTuner):
    ...
    def _build_and_fit_model(self, trial, *args, **kwargs):
        ...
        try:
            ...
            return results
        except errors.FailedTrialError:
            return float('inf') if self.oracle.objective.direction =='min' else -float('inf')
    
    def count_runs(self):
        memory_exceeded_count = 0
        done = 0
        for trial in self.oracle.trials.values():
            if trial.score in [float('inf'), -float('inf'), None]:
                memory_exceeded_count += 1
            else:
                done+=1
        # return {'failed': memory_exceeded_count, 'done':done}
        return memory_exceeded_count, done

    def search_atleast(self, num_trials, *search_args, **search_kwargs):
        done = 0
        num_without_better = search_kwargs.pop('num_without_better', 10)
        best_last = float('inf') if self.oracle.objective.direction == 'min' else -float('inf')
        self.oracle.max_trials = num_trials
        while done < num_trials:
            last_done = done
            self.search(*search_args, **search_kwargs)
            failed, done = self.count_runs()
            self.oracle.max_trials += failed
            best_now = self.oracle.get_best_trials(1)[0].score
            if done - last_done > num_without_better:
                print('num_without_better')
                if abs(best_last - best_now) < 1e-5:
                    print('break')
                    break
...
@keras_tuner_export("micronas.engine.tuner.maybe_distribute")
