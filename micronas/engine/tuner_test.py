...
import keras_tuner
from micronas import errors
from micronas.backend import config
from micronas.backend import keras
from micronas.engine import tuner as tuner_module
...
class MockHyperModel(micronas.HyperModel):
...
class ExampleHyperModel(micronas.HyperModel):
    def build(self, hp):
        inputs = keras.Input(shape=(INPUT_DIM,))
        x = inputs
        for i in range(hp.Int("num_layers", 1, 4)):
...
def test_basic_tuner_attributes(tmp_path):
    tuner = micronas.tuners.RandomSearch(
        build_model,
...
def test_no_hypermodel_with_objective(tmp_path):
    class MyTuner(micronas.tuners.RandomSearch):
...
def test_saving_and_reloading(tmp_path):
    ...
    new_tuner = micronas.tuners.RandomSearch(
        build_model,
        objective="val_accuracy",
        max_trials=4,
        executions_per_trial=2,
...
def test_update_trial(tmp_path):
    ...
    class MyOracle(micronas.Oracle):
...
def test_tunable_false_hypermodel(tmp_path):
    def build_model(hp):
        ...
        with hp.name_scope("xception"):
            ...
            xception = micronas.applications.HyperXception(
...
def test_get_best_hyperparameters(tmp_path):
    hp1 = micronas.HyperParameters()
    hp1.Fixed("a", 1)
    trial1 = micronas.engine.trial.Trial(hyperparameters=hp1)
    trial1.status = "COMPLETED"
    trial1.score = 10

    hp2 = micronas.HyperParameters()
    hp2.Fixed("a", 2)
    trial2 = micronas.engine.trial.Trial(hyperparameters=hp2)
...
def test_reloading_error_message(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="pass `overwrite=True`"):
        micronas.tuners.BayesianOptimization(
            build_model,
            objective="val_accuracy",
            max_trials=2,
            executions_per_trial=3,
...
@pytest.mark.skipif(
    micronas.backend.config.backend() != "tensorflow",
...
def test_convert_hyperparams_to_hparams():
    ...
    hps = micronas.engine.hyperparameters.HyperParameters()
    hps.Choice("learning_rate", [1e-4, 1e-3, 1e-2])
    hparams = micronas.engine.tuner_utils.convert_hyperparams_to_hparams(
        hps, hparams_api
    )
...
def test_tuning_correctness(tmp_path):
    tuner = micronas.Tuner(
        oracle=micronas.tuners.randomsearch.RandomSearchOracle(
...
def assert_found_best_score(
    tmp_path, hypermodel, tuner_class=micronas.Tuner
):
    tuner = tuner_class(
        oracle=micronas.tuners.randomsearch.RandomSearchOracle(
            objective="loss", max_trials=2, seed=1337
...
def test_metric_direction_inferred_from_objective(tmp_path):
    oracle = micronas.tuners.randomsearch.RandomSearchOracle(
        objective=micronas.Objective("a", "max"), max_trials=1
    )
    ...
    oracle = micronas.tuners.randomsearch.RandomSearchOracle(
        objective=micronas.Objective("a", "min"), max_trials=1
    )
...
def test_populate_initial_space_with_declare_hp(tmp_path):
    class MyHyperModel(micronas.HyperModel):
