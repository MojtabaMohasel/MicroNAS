...
from micronas.engine import objective as obj_module
from micronas.engine import stateful
...
@keras_tuner_export("micronas.synchronized")
...
@keras_tuner_export(["micronas.Oracle", "micronas.engine.oracle.Oracle"])
class Oracle(stateful.Stateful):
    ...
    Args:
        objective: A string, `micronas.Objective` instance, or a list of
            `micronas.Objective`s and strings. If a string, the direction of
            the optimization (min or max) will be inferred. If a list of
            `micronas.Objective`, we will minimize the sum of all the
    ...
    def end_trial(self, trial):
        ...
        if trial.status == trial_module.TrialStatus.COMPLETED:
            self.score_trial(trial)
            score = float('inf') if self.objective.direction =='min' else -float('inf')
            if np.isnan(trial.score) or trial.score== score:
